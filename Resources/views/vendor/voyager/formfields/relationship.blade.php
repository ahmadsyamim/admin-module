        ...
        @if ($options->type == 'belongsTo')
            ...
            @else
                @php
                    ...
                    if (!$query->count() && isset($options->default) && $options->default) {
                        $query = $model::where($options->key, $options->default)->get();
                    }
                    if (is_null($dataTypeContent->getKey()) && request()->get($options->column)) {
                        $query = $model::where($options->key, request()->get($options->column))->get();
                    }
                @endphp
                    ...
                    @foreach ($query as $relationshipData)
                        <option value="{{ $relationshipData->{$options->key} }}"
                            @if (old($options->column, $dataTypeContent->{$options->column}) == $relationshipData->{$options->key} ||
                                    (isset($options->default) && $options->default == $relationshipData->{$options->key})) selected="selected" @endif>
            ...
            @if (isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $selected_values = $model
                        ...
                        ->get()
                        ->mapWithKeys(function ($item, $key) use ($options) {
                            return [$item->{$options->key} => $item->{$options->label}];
                        })
                        ->all();
                @endphp
                ...
                @else
                    @can('add', $model)
                        <a href="{{ route('voyager.reviews.create', [$options->column => $relationshipData->id]) }}"
                            class="positive ui button">Add</a>
                    @endcan
                    @if (empty($selected_values))
                        <p>{{ __('voyager::generic.no_results') }}</p>
                    @else
                        <ul>
                            @foreach ($selected_values as $selected_id => $selected_value)
                                <li>{{ $selected_value }}
                                    @can('delete', $model)
                                        {{ \Form::open(['url' => route('voyager.reviews.destroy', $selected_id), 'method' => 'delete']) }}
                                        <input type="submit" class="negative ui button" value="Delete">
                                        {{ \Form::close() }}
                                    @endcan
                                </li>
                            @endforeach
